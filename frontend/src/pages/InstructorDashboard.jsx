...
export default function InstructorDashboard() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const myCreatedCourses = user?.role === "admin" 
        ? res.data.courses 
  ...
  const toggleCourseForm = () => {
    setShowForm((prev) => {
      const next = !prev;
      if (next) setFormData(initialFormData);
      return next;
    });
  };
  ...
  const handleUpdateStudent = async (enrollmentId, newProgress, newGrade, newStatus) => {
    try {
      await api.put(`/enroll/${enrollmentId}`, { progress: newProgress, grade: newGrade, status: newStatus });
                           ...
                           <li key={enrollment._id} style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', border: '1px solid var(--glass-border)'}}>
                             ...
                             <div style={{display: 'flex', gap: '0.75rem', alignItems: 'flex-end', flexWrap: 'wrap'}}>
                               <div style={{display: 'flex', flexDirection: 'column', gap: '4px'}}>
                                 ...
                                 <select defaultValue={enrollment.status} id={`status-${enrollment._id}`}
