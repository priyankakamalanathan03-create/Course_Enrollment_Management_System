...
{myCourses.map((enrollment) => {
  // Logic to determine the message and button text based on status
  let statusMessage = "";
  let buttonText = "Drop Course";
  ...
  switch (enrollment.status) {
    ...
    case "enrolled":
      ...
      break;
    case "rejected":
      statusMessage = "Sorry, your enrollment was not approved.";
      buttonText = "Remove from List";
      showProgress = false;
      break;
    case "waitlisted":
      statusMessage = "Course is full. You are on the waitlist.";
      buttonText = "Leave Waitlist";
      showProgress = false;
      break;
    case "completed":
      statusMessage = "Congratulations! You finished this course.";
  ...
  return (
    <motion.div key={enrollment._id} className="glass-panel course-card flex flex-col">
      ...
      <div className="course-header">
        <h3 className="course-title">{enrollment.courseId?.title || "Course Deleted"}</h3>
      ...
      {/* Correct Message */}
      <p style={{ 
        color: enrollment.status === 'rejected' ? 'var(--danger)' : 'var(--warning)', 
        fontSize: '0.85rem', 
        margin: '1rem 0',
        fontWeight: '500'
      }}>
      ...
      {/* Only show progress if ENROLLED or COMPLETED */}
      {showProgress && (
      ...
      <motion.button 
        ...
        style={{
          ...
          border: `1px solid ${enrollment.status === 'rejected' ? 'var(--text-secondary)' : '#ef4444'}`, 
          color: enrollment.status === 'rejected' ? 'var(--text-secondary)' : '#ef4444', 
          padding: '0.5rem', borderRadius: '6px', cursor: 'pointer', marginTop: showProgress ? '0' : 'auto'
        }}
      >
        <XCircle size={16} /> 
        {buttonText}
